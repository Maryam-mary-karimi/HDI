...
public class Tree implements Serializable{
	...
	public static Vector<DataNode> sort(Vector<DataNode> arr)
	{
		int n = arr.size();
		for (int i=1; i<n; ++i)
		{
			DataNode key = arr.elementAt(i);
			int j = i-1;

			/* Move elements of arr[0..i-1], that are greater than key, to one position ahead of their current position */
			while (j>=0 && ( arr.elementAt(j).device.deviceName.compareTo(key.device.deviceName)>0 
					|| (arr.elementAt(j).device.deviceName.compareTo(key.device.deviceName)==0 && arr.elementAt(j).date.year>key.date.year )))
			{
	...
	public DataNode addParent(int level, Vector<DataNode> children) {
		...
		case 2:
			...
			break;
		case 1:
			n=new DataNode("root", new Date(0,0,0) , new Device("allDevices","localhost",0) , valueForParent);
			numOfNodes++;
			break;
		default:
	...
	public void PrintTree(FileWriter fw){
		try {
			//if(PGW_Main.iflog)System.out.println("___print____");
	...
	public void printTreeNodes(DataNode datanode, FileWriter fw){
		try {
			if(datanode.id.equals("|")){
				//if(PGW_Main.iflog)System.out.print("|");
				fw.write("|");
			}
			else{
				//if(PGW_Main.iflog)System.out.print(datanode.id+" "+datanode.date.year+" "+datanode.device.deviceName+" * ");
	...
	public Vector<DataNode> printArray(Vector<DataNode> nodes, FileWriter fw){
		...
		//if(PGW_Main.iflog)System.out.println();//fw ro bade seda zadan method gozashtam
		return children;
	}
