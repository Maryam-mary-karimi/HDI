...
public class P_GW{
	...
	public BloomFilter<String> addDataToBloomFilter(DataNode [] leaves, int tempbfsize){
	//	try {
			...
			for(int i=0; i<leaves.length;i++){
				...
				bf.add(tempBf.StringOfBits(), true);//hash that bloom filter in a main bloom filter
	...
	public Tree updateTree(DataNode[] newleaves, Tree oldtree){
	...
	public boolean verifyData(String[] bf2,DataNode[] SHAnodes,Vector<DataNode> relatedNodesInTree) throws java.rmi.RemoteException {
		//try {
		...
		try {
			...
			while(true) {
				...
				if(m.getType().equals("verify")){
					...
					boolean verification=pgw.verifyData(m.bf2,m.SHAnodes,m.getRelatedDataNodesinTree());
					long time2s= System.nanoTime();
					PGW_Main.allPrints= PGW_Main.allPrints+("verification time in PGW: "+(time2s-time1s)/1000+" Micro second \n");
...
class Update extends Thread{
	...
	public void  run() {
		try {		
			...
			DataNode[] temp=new DataNode[0];


			for (int i = 0; i < pgw.devices.length; i++) {
				if(pgw.devices[i].deviceName.equals("HRM")){
					temp = pgw.devices[i].readDataPacket("HRMupdate");
				}
			}

			DataNode[] allnewdataset= {temp[0]};
			//////////////////// P GateWay //////////////////////////////
			...
			pgw.addDataToBloomFilter(allnewdataset, tmpBfSize);
			...
			Tree newtree=pgw.updateTree(newleaves,oldTree);

			long time2s= System.currentTimeMillis(); 

			System.out.println("required time to update tree "+(time2s-time1s)+" ms ");
			PGW_Main.allPrints=PGW_Main.allPrints+("required time to update tree "+(time2s-time1s)+" ms \n");
			...
			for (int i = 0; i < pgw.devices.length; i++) {
				...
				//boolean successUpdate=
						PGW_Main.SendToServer(treeMessage,pgw.devices[i].cloudIP,pgw.devices[i].cloudport);//.getAck();
			...
			PGW_Main.allPrints=PGW_Main.allPrints+("required time to update "+(timeupdate2-timeupdate1)+" ms \n");
			System.out.println("required time to update and send "+(timeupdate3-timeupdate1)+" ms ");
			PGW_Main.allPrints=PGW_Main.allPrints+("required time to update and send "+(timeupdate3-timeupdate1)+" ms \n");
